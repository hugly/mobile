/**
 * Created by hulgy on 19/11/2016.
 */
(function(){
    'use strict';
    ...
    require(['mmRequest','domReady!'],function(avalon){
        var vm=avalon.define({
            ...
            chooseAddress:function(type){
                ...
                if(vm.isSame === 'true'){
                    vm.urlData=jsonInList({'pickupPoiLat':vm.nowLat},vm.urlData);
                    vm.urlData=jsonInList({'pickupPoiLon':vm.nowLng},vm.urlData);
                    vm.urlData=jsonInList({'pickupAddress':vm.address},vm.urlData);
                    vm.urlData=jsonInList({'pickupPoiName':vm.shortaddress},vm.urlData);
                    vm.urlData=jsonInList({'pickupPoiCode':vm.addressCode},vm.urlData);
                }else{
                    vm.urlData=jsonInList({'deliveryPoiLat':vm.nowLat},vm.urlData);
                    vm.urlData=jsonInList({'deliveryPoiLon':vm.nowLng},vm.urlData);
                    vm.urlData=jsonInList({'deliveryAddress':vm.address},vm.urlData);
                    vm.urlData=jsonInList({'deliveryPoiName':vm.shortaddress},vm.urlData);
                    vm.urlData=jsonInList({'deliveryPoiCode':vm.addressCode},vm.urlData);
                }

                vm.searchData=transList2Url(vm.urlData.$model);

                window.location.href = "offline_info.html?"+vm.searchData;
            },
            //检测历史记录是否超长,并作出处理
